...
# echo $(oc1 get configmap -n mesh1-system istio-ca-root-cert -o jsonpath='{.data.root-cert\.pem}' | sed ':a;N;$!ba;s/\n/\\\n    /g') > import/mesh1_cert.pem
...
# echo $(oc2 get configmap -n mesh2-system istio-ca-root-cert -o jsonpath='{.data.root-cert\.pem}' | sed ':a;N;$!ba;s/\n/\\\n    /g') > export/mesh2_cert.pem
...
# INFO: sed and envsubst not indenting the cert
# sed "s:{{MESH2_CERT}}:'$MESH2_CERT':g" export/configmap.yaml | oc1 apply -f -
# envsubst < export/configmap.yaml | oc1 apply -f -
# echo $MESH2_CERT > export/mesh2_cert.pem
...
# envsubst < import/configmap.yaml | oc2 apply -f -
# echo $MESH1_CERT > import/mesh1_cert.pem
...
export BOOKINFO_GIT_URL=https://raw.githubusercontent.com/maistra/istio/maistra-2.1
oc1 -n mesh1-bookinfo apply -f "${BOOKINFO_GIT_URL}"/samples/bookinfo/platform/kube/bookinfo.yaml
oc1 -n mesh1-bookinfo apply -f "${BOOKINFO_GIT_URL}"/samples/bookinfo/platform/kube/bookinfo-ratings-v2.yaml
oc1 -n mesh1-bookinfo apply -f "${BOOKINFO_GIT_URL}"/samples/bookinfo/platform/kube/bookinfo-db.yaml
oc1 -n mesh1-bookinfo apply -f "${BOOKINFO_GIT_URL}"/samples/bookinfo/networking/destination-rule-all.yaml

log "Installing bookinfo in mesh2"
oc2 -n mesh2-bookinfo apply -f "${BOOKINFO_GIT_URL}"/samples/bookinfo/platform/kube/bookinfo.yaml
oc2 -n mesh2-bookinfo apply -f "${BOOKINFO_GIT_URL}"/samples/bookinfo/platform/kube/bookinfo-ratings-v2.yaml
oc2 -n mesh2-bookinfo apply -f "${BOOKINFO_GIT_URL}"/samples/bookinfo/networking/bookinfo-gateway.yaml
oc2 -n mesh2-bookinfo apply -f "${BOOKINFO_GIT_URL}"/samples/bookinfo/networking/destination-rule-all.yaml
oc2 -n mesh2-bookinfo apply -f "${BOOKINFO_GIT_URL}"/samples/bookinfo/networking/virtual-service-reviews-v3.yaml
